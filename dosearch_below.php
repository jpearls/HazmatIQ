...
if(!isset($_GET['id'])) {
	//echo 'Query!';
	$sql = "SELECT id, number, name FROM below ORDER BY number";
} else {
	//echo 'Id!';
	$id = $_GET['id'];
	$sql = 'SELECT * FROM below WHERE id = "' . $id . '"';
	$query = "";
}
...
if($num_results == 0) {
	...
	while ($row = mysql_fetch_array($result)) {
		$display .= '
			<li><a href="above.php?t=below&id=' . $row['id'] . '">Blue ' . $row['number'] . '. ' . $row['name'] . '</a></li>
		';
	}
	...
	while ($data = mysql_fetch_array($result)) {
		...
		$display = '
			<div style="width:100%; text-align:center;">
				<span style="color:#0A90CF;font-size:32px;font-weight:bold;">Blue ' . $data['number'] . '. ' . $data['name'] . '</span>		
			</div>
			...
			<div style="width:100%;">
				<table style="width:300px; border:1px solid red;margin:auto;background-color:#E6937C;" border="1">  <!--background-color:#E6937C;-->
					...
					<tr>
						...
						<th><span style="font-size:12px;line-height:12px;"><span style="color:yellow;">wet</span> &bull; dry <br/> F &bull; pH <br/> corrosive</span></th>
					...
					<tr>
						...
						<th>' . $corrosive . '<br/><span style="font-size:12px;">' . $data['corrosive_notes'] . '</span></th>
						<th>' . $data['tempgun'] . '</th>
						<th>' . $data['cgi'] . '</th>
					</tr>
				</table>
			...
			<br/>
			<div style="width:100%;">
				<table style="width:300px; border:1px solid green;margin:auto;background-color:#94C789;" border="1">
					<tr>
